...
namespace ObbedNews.Controllers.News
{
    ...
    public class NewsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Models.News>>> GetNews(
            ...
            [FromQuery] bool withoutEmpty = true)
        {
            ...
            {
                case false:
                    q = q.Include(c => c.VideoSections);
                    q = q.Include(c => c.Category);
                    break;
                case true:
                    q = q.Where(c => c.VideoSections.Count > 0);
                    break;
            }
        ...
        )
        {
            ...
            if (await _context.News.AnyAsync(m => m.Slug == dto.Slug))
            {
                return Conflict();
            }
            ...
            [FromRoute] long newsId)
        {
            ...
            if (isExists)
            {
                return BadRequest($"News Id: \"{newsId}\" - Videos Already In DB");
            }
            ...
            if (server is null)
            {
                return BadRequest("Server not set");
            }
            ...
            {
                res = await client
                    .GetFromJsonAsync<ResponseVideoFoldersDto>("/get-video-map.php?courseId=" + newsId);
            }
            catch (HttpRequestException e)
            {
                if (e.StatusCode == HttpStatusCode.Unauthorized)
                {
                    return Unauthorized("Video Server Not Authorized");
                }
            }
            catch (Exception e)
            {
